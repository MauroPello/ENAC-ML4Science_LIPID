...
def _extract_scores(model: Pipeline, X_test: pd.DataFrame) -> np.ndarray | None:
    """Retrieve probability or decision scores for AUC calculation."""
    try:
    ...
    if hasattr(final_estimator, "predict_proba"):
        try:
            return final_estimator.predict_proba(X_test)[:, 1]
        except Exception:
            return None
    if hasattr(final_estimator, "decision_function"):
        try:
            return final_estimator.decision_function(X_test)
        except Exception:
            return None
    return None
    ...
    *,
    ...
    for model_name, model in binary_models.items():
        ...
        if len(np.unique(y_test)) == 2:
            y_scores = _extract_scores(model, X_test)
    ...
    classification_df = (
        pd.DataFrame(classification_results).sort_values(by="F1", ascending=False)
        if classification_results
        else pd.DataFrame()
    )

    return {
        "classification_results": classification_df,

...
class ResultsVisualizer:
    ...
    def summary(self) -> None:
        ...
        if df is not None and not df.empty:
            ...
            for _, row in df.iterrows():
                ...
                print(
                    f"  {row.get('model', 'Unknown'):25s} | {metric} = {val:.4f}"
                    if isinstance(val, float)
                    else f"  {row.get('model')}"
                )
    ...
    def _plot_regression(self, figsize: tuple) -> None:
        ...
        n_plots = sum(
            [
                reg_df is not None,
                coef_df is not None and not coef_df.empty,
                bool(residuals),
            ]
        )
        if n_plots == 0:
        ...
        fig.suptitle(
            f"{self.experiment_name} ({self.target_variable})", fontweight="bold"
        )
        ...
        if residuals and (best := self.results.get("best_model_name")) in residuals:
            ...
            axes[idx].set(
                xlabel="Predicted", ylabel="Residuals", title=f"Residuals - {best}"
            )
...
def visualize_experiment(
    experiment_name: str, base_dir: str = "results"
) -> ResultsVisualizer:
...
def compare_targets(
    target_list: List[str], metric: str = "R2", base_dir: str = "results"
) -> None:
    ...
    for target in target_list:
        ...
        df = latest.get(
            "regression_results"
            if latest.get("target_type") == "continuous"
            else "classification_results"
        )
        ...
        data.append(
            {"Target": target, "Best Model": best[model_col], metric: best[metric]}
        )
    ...
    for i, row in df.iterrows():
        ax.text(
            row[metric],
            i,
            f" {row['Best Model']} ({row[metric]:.3f})",
            va="center",
            fontsize=9,
        )
    plt.tight_layout()
